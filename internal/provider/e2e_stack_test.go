...
func (d *e2eDocker) mintProxyToken(ctx context.Context) (string, error) {
	out, err := d.exec(ctx, d.server,
		"/go/bin/netbird-server", "token", "create", "--name", "e2e-proxy", "--config", "/nb/config.yaml")
	if err != nil {
		return "", fmt.Errorf("mint a proxy access token: %w: %s", err, out)
	}
...
// exec runs a command in one of the deployment's containers and returns its
// combined output. Used to drive the server's own CLI during bootstrap, and by
// tests that need to assert on what a container reaches rather than what the
// host reaches.
